...
// =============================================================================
// pointTask() — core logic of pointing (posizione-only)
// =============================================================================
void CartesianPointingComponent::pointTask(const std::shared_ptr<cartesian_pointing_interfaces::srv::PointAt::Request> request)
{
    ...
    if (it == m_artworkCoords.end() || it->second.size() < 3) {
        RCLCPP_WARN(m_node->get_logger(), "No or invalid ARTWORK '%s' found", request->target_name.c_str());
        return;
    }
    ...
    //             request->target_name.c_str(), m_mapFrame.c_str(),
    ...
    if (transformPointMapToRobot(mapPt, basePt, m_baseFrame, 1.0)) {
        ...
        // RCLCPP_DEBUG(m_node->get_logger(),
        //             "[TARGET] '%s' in %s : (%.3f, %.3f, %.3f)",
        //             request->target_name.c_str(), m_baseFrame.c_str(),
...
// =============================================================================
std::map<std::string, std::vector<double>> CartesianPointingComponent::loadArtworkCoordinates(const std::string& filename)
{
