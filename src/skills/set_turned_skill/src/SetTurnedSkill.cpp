...
bool SetTurnedSkill::start(int argc, char*argv[])
{
  ...
  nodeIncreaseTurnBacksCounter = rclcpp::Node::make_shared(m_name + "SkillNodeIncreaseTurnBacksCounter");
  clientIncreaseTurnBacksCounter = nodeIncreaseTurnBacksCounter->create_client<turn_back_manager_interfaces::srv::IncreaseTurnBacksCounter>("/TurnBackManagerComponent/IncreaseTurnBacksCounter");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'TurnBackManagerComponent/IncreaseTurnBacksCounter' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("TurnBackManagerComponent.IncreaseTurnBacksCounter.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'IncreaseTurnBacksCounter'.");
      }
      QVariantMap data;
  ...
  nodeSetString = rclcpp::Node::make_shared(m_name + "SkillNodeSetString");
  clientSetString = nodeSetString->create_client<blackboard_interfaces::srv::SetStringBlackboard>("/BlackboardComponent/SetString");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'BlackboardComponent/SetString' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("BlackboardComponent.SetString.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'SetString'.");
      }
      QVariantMap data;
      data.insert("call_succeeded", false);
      m_stateMachine.submitEvent("BlackboardComponent.SetString.Return", data);
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "SetTurnedSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
