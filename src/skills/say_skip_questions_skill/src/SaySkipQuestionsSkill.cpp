...
SaySkipQuestionsSkill::SaySkipQuestionsSkill(std::string name ) :
		m_name(std::move(name))
{
    
}

SaySkipQuestionsSkill::~SaySkipQuestionsSkill()
{
    //std::cout << "DEBUG: Invoked destructor of SaySkipQuestionsSkill" << std::endl;
    m_threadSpin->join();
}

void SaySkipQuestionsSkill::spin(std::shared_ptr<rclcpp::Node> node)
{
    rclcpp::spin(node);
    rclcpp::shutdown();
    QCoreApplication::quit();
    //std::cout << "DEBUG: SaySkipQuestionsSkill::spin successfully ended" << std::endl;
}

bool SaySkipQuestionsSkill::start(int argc, char*argv[])
{
	if(!rclcpp::ok())
	{
	...
	RCLCPP_DEBUG_STREAM(m_node->get_logger(), "SaySkipQuestionsSkill::start");
	std::cout << "DEBUG: SaySkipQuestionsSkill::start" << std::endl;

  
	m_tickService = m_node->create_service<bt_interfaces_dummy::srv::TickAction>(m_name + "Skill/tick",
                                                                           	std::bind(&SaySkipQuestionsSkill::tick,
                                                                           	this,
                                                                           	std::placeholders::_1,
                                                                           	std::placeholders::_2));
  m_tickService->configure_introspection(m_node->get_clock(), rclcpp::SystemDefaultsQoS(), RCL_SERVICE_INTROSPECTION_CONTENTS);
	...
	m_haltService = m_node->create_service<bt_interfaces_dummy::srv::HaltAction>(m_name + "Skill/halt",
                                                                            	std::bind(&SaySkipQuestionsSkill::halt,
                                                                            	this,
                                                                            	std::placeholders::_1,
                                                                            	std::placeholders::_2));
  ...
  m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "SaySkipQuestionsSkill::tickReturn %s", event.data().toMap()["status"].toString().toStdString().c_str());
  ...
  m_stateMachine.connectToEvent("HALT_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "SaySkipQuestionsSkill::haltresponse");
...
void SaySkipQuestionsSkill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Request> request,
                                std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Response>      response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  RCLCPP_INFO(m_node->get_logger(), "SaySkipQuestionsSkill::tick");
  ...
  RCLCPP_INFO(m_node->get_logger(), "SaySkipQuestionsSkill::tickDone");
  response->is_ok = true;
}

void SaySkipQuestionsSkill::halt( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::HaltAction::Request> request,
    [[maybe_unused]] std::shared_ptr<bt_interfaces_dummy::srv::HaltAction::Response> response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  RCLCPP_INFO(m_node->get_logger(), "SaySkipQuestionsSkill::halt");
  m_haltResult.store(false);
  m_stateMachine.submitEvent("CMD_HALT");
  while(!m_haltResult.load()) {
      std::this_thread::sleep_for (std::chrono::milliseconds(100));
  }
  RCLCPP_INFO(m_node->get_logger(), "SaySkipQuestionsSkill::haltDone");
  response->is_ok = true;
}
