...
IsPoiDone10Skill::IsPoiDone10Skill(std::string name ) :
		m_name(std::move(name))
{
    
}

IsPoiDone10Skill::~IsPoiDone10Skill()
{
    //std::cout << "DEBUG: Invoked destructor of IsPoiDone10Skill" << std::endl;
    m_threadSpin->join();
}

void IsPoiDone10Skill::spin(std::shared_ptr<rclcpp::Node> node)
{
    rclcpp::spin(node);
    rclcpp::shutdown();
    QCoreApplication::quit();
    //std::cout << "DEBUG: IsPoiDone10Skill::spin successfully ended" << std::endl;
}

bool IsPoiDone10Skill::start(int argc, char*argv[])
{
	if(!rclcpp::ok())
	{
	...
	RCLCPP_DEBUG_STREAM(m_node->get_logger(), "IsPoiDone10Skill::start");
	std::cout << "DEBUG: IsPoiDone10Skill::start" << std::endl;

  
	m_tickService = m_node->create_service<bt_interfaces_dummy::srv::TickCondition>(m_name + "Skill/tick",
                                                                           	std::bind(&IsPoiDone10Skill::tick,
                                                                           	this,
                                                                           	std::placeholders::_1,
                                                                           	std::placeholders::_2));
  ...
  m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "IsPoiDone10Skill::tickReturn %s", event.data().toMap()["status"].toString().toStdString().c_str());
...
void IsPoiDone10Skill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Request> request,
                                std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Response>      response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  RCLCPP_INFO(m_node->get_logger(), "IsPoiDone10Skill::tick");
  ...
  RCLCPP_INFO(m_node->get_logger(), "IsPoiDone10Skill::tickDone");
  RCLCPP_DEBUG(m_node->get_logger(), "IsPoiDone10Skill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
