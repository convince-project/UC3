  ...
  m_stateMachine.connectToEvent("NavigationComponent.GetNavigationStatus.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          auto response = result.get();
           ...
           RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "GetNavigationStatus response: is_ok=%s, status=%d", response->is_ok ? "true" : "false", response->status.status);
           RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "status=%d", response->status.status);
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "IsAtChargingStationSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
