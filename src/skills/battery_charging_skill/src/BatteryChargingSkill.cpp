...
bool BatteryChargingSkill::start(int argc, char*argv[])
{
  ...
  m_subscription_battery_charging = m_node->create_subscription<sensor_msgs::msg::BatteryState>(
  "/BatteryComponent/battery_charging", 10, std::bind(&BatteryChargingSkill::topic_callback_battery_charging, this, std::placeholders::_1));
...
void BatteryChargingSkill::topic_callback_battery_charging(const sensor_msgs::msg::BatteryState::SharedPtr msg) {
  std::cout << "callback" << std::endl;
  QVariantMap data;
  
  data.insert("power_supply_status", msg->power_supply_status);
  
  m_stateMachine.submitEvent("BatteryComponent.battery_charging.Sub", data);
  RCLCPP_INFO(m_node->get_logger(), "BatteryComponent.battery_charging.Sub");
}
