...
IsPoiDone3Skill::IsPoiDone3Skill(std::string name ) :
		m_name(std::move(name))
{
    
}

IsPoiDone3Skill::~IsPoiDone3Skill()
{
    //std::cout << "DEBUG: Invoked destructor of IsPoiDone3Skill" << std::endl;
    m_threadSpin->join();
}

void IsPoiDone3Skill::spin(std::shared_ptr<rclcpp::Node> node)
{
    rclcpp::spin(node);
    rclcpp::shutdown();
    QCoreApplication::quit();
    //std::cout << "DEBUG: IsPoiDone3Skill::spin successfully ended" << std::endl;
}

bool IsPoiDone3Skill::start(int argc, char*argv[])
{
	if(!rclcpp::ok())
	{
	...
	RCLCPP_DEBUG_STREAM(m_node->get_logger(), "IsPoiDone3Skill::start");
	std::cout << "DEBUG: IsPoiDone3Skill::start" << std::endl;

  
	m_tickService = m_node->create_service<bt_interfaces_dummy::srv::TickCondition>(m_name + "Skill/tick",
                                                                           	std::bind(&IsPoiDone3Skill::tick,
                                                                           	this,
                                                                           	std::placeholders::_1,
                                                                           	std::placeholders::_2));
  ...
  m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "IsPoiDone3Skill::tickReturn %s", event.data().toMap()["status"].toString().toStdString().c_str());
...
void IsPoiDone3Skill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Request> request,
                                std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Response>      response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  RCLCPP_INFO(m_node->get_logger(), "IsPoiDone3Skill::tick");
  ...
  RCLCPP_INFO(m_node->get_logger(), "IsPoiDone3Skill::tickDone");
  response->is_ok = true;
}
