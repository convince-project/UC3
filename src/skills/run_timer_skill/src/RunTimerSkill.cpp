...
bool RunTimerSkill::start(int argc, char*argv[])
{
  ...
  nodeStartTimer = rclcpp::Node::make_shared(m_name + "SkillNodeStartTimer");
  clientStartTimer = nodeStartTimer->create_client<timer_check_for_people_interfaces::srv::StartTimer>("/TimerCheckForPeopleComponent/StartTimer");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'TimerCheckForPeopleComponent/StartTimer' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("TimerCheckForPeopleComponent.StartTimer.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'StartTimer'.");
      }
      QVariantMap data;
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "RunTimerSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
