...
SetPoi3Skill::SetPoi3Skill(std::string name ) :
		m_name(std::move(name))
{
    
}

SetPoi3Skill::~SetPoi3Skill()
{
    //std::cout << "DEBUG: Invoked destructor of SetPoi3Skill" << std::endl;
    m_threadSpin->join();
}

void SetPoi3Skill::spin(std::shared_ptr<rclcpp::Node> node)
{
    rclcpp::spin(node);
    rclcpp::shutdown();
    QCoreApplication::quit();
    //std::cout << "DEBUG: SetPoi3Skill::spin successfully ended" << std::endl;
}

bool SetPoi3Skill::start(int argc, char*argv[])
{
	if(!rclcpp::ok())
	{
  ...
  m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "SetPoi3Skill::tickReturn %s", event.data().toMap()["status"].toString().toStdString().c_str());
  ...
  m_stateMachine.connectToEvent("HALT_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "SetPoi3Skill::haltresponse");
...
void SetPoi3Skill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Request> request,
                                std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Response>      response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  RCLCPP_INFO(m_node->get_logger(), "SetPoi3Skill::tick");
  ...
  RCLCPP_INFO(m_node->get_logger(), "SetPoi3Skill::tickDone");
  response->is_ok = true;
}

void SetPoi3Skill::halt( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::HaltAction::Request> request,
    [[maybe_unused]] std::shared_ptr<bt_interfaces_dummy::srv::HaltAction::Response> response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  RCLCPP_INFO(m_node->get_logger(), "SetPoi3Skill::halt");
  m_haltResult.store(false);
  m_stateMachine.submitEvent("CMD_HALT");
  while(!m_haltResult.load()) {
      std::this_thread::sleep_for (std::chrono::milliseconds(100));
  }
  RCLCPP_INFO(m_node->get_logger(), "SetPoi3Skill::haltDone");
  response->is_ok = true;
}
