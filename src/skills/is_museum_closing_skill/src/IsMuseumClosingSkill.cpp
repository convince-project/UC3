...
bool IsMuseumClosingSkill::start(int argc, char*argv[])
{
  ...
  nodeIsMuseumClosing = rclcpp::Node::make_shared(m_name + "SkillNodeIsMuseumClosing");
  clientIsMuseumClosing = nodeIsMuseumClosing->create_client<time_interfaces::srv::IsMuseumClosing>("/TimeComponent/IsMuseumClosing");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'TimeComponent/IsMuseumClosing' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("TimeComponent.IsMuseumClosing.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'IsMuseumClosing'.");
      }
      QVariantMap data;
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "IsMuseumClosingSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
