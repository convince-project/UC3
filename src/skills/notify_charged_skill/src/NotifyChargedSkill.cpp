...
bool NotifyChargedSkill::start(int argc, char*argv[])
{
  ...
  nodeNotifyUserCharged = rclcpp::Node::make_shared(m_name + "SkillNodeNotifyUserCharged");
  clientNotifyUserCharged = nodeNotifyUserCharged->create_client<notify_user_interfaces::srv::NotifyUserCharged>("/NotifyUserComponent/NotifyUserCharged");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'NotifyUserComponent/NotifyUserCharged' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("NotifyUserComponent.NotifyUserCharged.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'NotifyUserCharged'.");
      }
      QVariantMap data;
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "NotifyChargedSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
