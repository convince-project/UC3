...
bool IsAllowedToMoveSkill::start(int argc, char*argv[])
{
    ...
    nodeIsAllowedToMove = rclcpp::Node::make_shared(m_name + "SkillNodeIsAllowedToMove");
    clientIsAllowedToMove = nodeIsAllowedToMove->create_client<allowed_to_move_interfaces::srv::IsAllowedToMove>("/AllowedToMoveComponent/IsAllowedToMove");
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'IsAllowedToMoveComponent/IsAllowedToMove' not available.");
        std::exit(1);
    }
  ...
  m_stateMachine.connectToEvent("AllowedToMoveComponent.IsAllowedToMove.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
    ...
    if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
    {
        ...
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'IsAllowedToMove'.");
    }
      QVariantMap data;
