...
bool IsAllowedToMoveSkill::start(int argc, char*argv[])
{
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'AllowedToMoveComponent/IsAllowedToMove' not available.");
        std::exit(1);
    }
  }
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "IsAllowedToMoveSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
