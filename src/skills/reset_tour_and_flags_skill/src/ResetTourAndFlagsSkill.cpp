...
bool ResetTourAndFlagsSkill::start(int argc, char*argv[])
{
  ...
  nodeResetPlanner = rclcpp::Node::make_shared(m_name + "SkillNodeResetPlanner");
  clientResetPlanner = nodeResetPlanner->create_client<std_srvs::srv::Trigger>("/PlannerComponent/ResetPlanner");
  clientResetPlanner->configure_introspection(nodeResetPlanner->get_clock(), rclcpp::SystemDefaultsQoS(), RCL_SERVICE_INTROSPECTION_CONTENTS);
  
  {
    bool wait_succeded{true};
    int retries = 0;
    while (!clientResetPlanner->wait_for_service(std::chrono::seconds(1))) {
        if (!rclcpp::ok()) {
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Interrupted while waiting for the service 'ResetPlanner'. Exiting.");
            wait_succeded = false;
            break;
        } 
        retries++;
        if(retries == SERVICE_TIMEOUT) {
            RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Timed out while waiting for the service 'ResetPlanner'.");
            wait_succeded = false;
            break;
        }
    }
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'PlannerComponent/ResetPlanner' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("PlannerComponent.ResetPlanner.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      auto request = std::make_shared<std_srvs::srv::Trigger::Request>();
      auto eventParams = event.data().toMap();
      
      auto result = clientResetPlanner->async_send_request(request);
      const std::chrono::seconds timeout_duration(SERVICE_TIMEOUT);
      auto futureResult = rclcpp::spin_until_future_complete(nodeResetPlanner, result, timeout_duration);
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          auto response = result.get();
           QVariantMap data;
           data.insert("call_succeeded", true);
           data.insert("success", response->success);
           m_stateMachine.submitEvent("PlannerComponent.ResetPlanner.Return", data);
           RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "PlannerComponent.ResetPlanner.Return");
           return;
      }
      else if(futureResult == rclcpp::FutureReturnCode::TIMEOUT){
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Timed out while future complete for the service 'ResetPlanner'.");
      }
      else {
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'ResetPlanner'.");
      }
      QVariantMap data;
      data.insert("call_succeeded", false);
      m_stateMachine.submitEvent("PlannerComponent.ResetPlanner.Return", data);
  ...
  clientSetAllIntsWithPrefix = nodeSetAllIntsWithPrefix->create_client<blackboard_interfaces::srv::SetAllIntsWithPrefixBlackboard>("/BlackboardComponent/SetAllIntsWithPrefix");
