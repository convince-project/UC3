...
bool IsAllowedToTurnBackSkill::start(int argc, char*argv[])
{
	...
	m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
		RCLCPP_INFO(m_node->get_logger(), "IsAllowedToTurnBackSkill::tickReturn %s", event.data().toMap()["result"].toString().toStdString().c_str());
...
void IsAllowedToTurnBackSkill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces::srv::TickCondition::Request> request,
