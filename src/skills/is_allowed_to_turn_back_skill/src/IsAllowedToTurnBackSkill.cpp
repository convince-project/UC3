...
bool IsAllowedToTurnBackSkill::start(int argc, char*argv[])
{
  ...
  nodeIsAllowedToTurnBack = rclcpp::Node::make_shared(m_name + "SkillNodeIsAllowedToTurnBack");
  clientIsAllowedToTurnBack = nodeIsAllowedToTurnBack->create_client<turn_back_manager_interfaces::srv::IsAllowedToTurnBack>("/TurnBackManagerComponent/IsAllowedToTurnBack");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'TurnBackManagerComponent/IsAllowedToTurnBack' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("TurnBackManagerComponent.IsAllowedToTurnBack.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'IsAllowedToTurnBack'.");
      }
      QVariantMap data;
  ...
  nodeGetString = rclcpp::Node::make_shared(m_name + "SkillNodeGetString");
  clientGetString = nodeGetString->create_client<blackboard_interfaces::srv::GetStringBlackboard>("/BlackboardComponent/GetString");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'BlackboardComponent/GetString' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("BlackboardComponent.GetString.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'GetString'.");
      }
      QVariantMap data;
      data.insert("call_succeeded", false);
      m_stateMachine.submitEvent("BlackboardComponent.GetString.Return", data);
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickCondition::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "IsAllowedToTurnBackSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
