...
bool StartServiceSkill::start(int argc, char*argv[])
{
  ...
  nodeStartService = rclcpp::Node::make_shared(m_name + "SkillNodeStartService");
  clientStartService = nodeStartService->create_client<manage_service_interfaces::srv::StartService>("/ManagePeopleDetectorComponent/StartService");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'ManagePeopleDetectorComponent/StartService' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("ManagePeopleDetectorComponent.StartService.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'StartService'.");
      }
      QVariantMap data;
                                ...
                                std::shared_ptr<bt_interfaces_dummy::srv::TickAction::Response>      response)
{
  ...
  RCLCPP_DEBUG(m_node->get_logger(), "StartServiceSkill num_retry: %d tick time: %ld", load_counter, duration_ms);
  response->is_ok = true;
}
