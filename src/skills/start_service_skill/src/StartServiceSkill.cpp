...
bool StartServiceSkill::start(int argc, char*argv[])
{
    ...
    m_stateMachine.connectToEvent("ManagePeopleDetectorComponent.StartService.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
        ...
        while (!clientStartService->wait_for_service(std::chrono::seconds(1))) {
            if (!rclcpp::ok()) {
                ...
                wait_succeded = false;
                m_stateMachine.submitEvent("ManagePeopleDetectorComponent.StartService.Return");
            } 
        }
        if (wait_succeded) {
            // send the request                                                                    
            auto result = clientStartService->async_send_request(request);
            auto futureResult = rclcpp::spin_until_future_complete(nodeStartService, result);
	...
	m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
		RCLCPP_INFO(m_node->get_logger(), "StartServiceSkill::tickReturn %s", event.data().toMap()["result"].toString().toStdString().c_str());
...
void StartServiceSkill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces::srv::TickAction::Request> request,
...
void StartServiceSkill::halt( [[maybe_unused]] const std::shared_ptr<bt_interfaces::srv::HaltAction::Request> request,
