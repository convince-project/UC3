...
bool SetNavigationPositionSkill::start(int argc, char*argv[])
{
  ...
  nodeIsDancing = rclcpp::Node::make_shared(m_name + "SkillNodeIsDancing");
  clientIsDancing = nodeIsDancing->create_client<execute_dance_interfaces::srv::IsDancing>("/ExecuteDanceComponent/IsDancing");
  ...
  
  {
    ...
    if (!wait_succeded) {
        RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Service 'ExecuteDanceComponent/IsDancing' not available.");
        std::exit(1);
    }
  }
  m_stateMachine.connectToEvent("ExecuteDanceComponent.IsDancing.Call", [this]([[maybe_unused]]const QScxmlEvent & event){
      ...
      if (futureResult == rclcpp::FutureReturnCode::SUCCESS) 
      {
          ...
          RCLCPP_ERROR(rclcpp::get_logger("rclcpp"), "Failed to call service 'IsDancing'.");
      }
      QVariantMap data;
