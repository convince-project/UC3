...
class ROSMonitor_monitor_prop11(Node):
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['/amcl_pose']={'package': 'geometry_msgs.msg', 'type': 'PoseWithCovarianceStamped'}
		self.topics_info['/monitoring_clock']={'package': 'builtin_interfaces.msg', 'type': 'Time'}
		self.config_subscribers['/amcl_pose']=self.create_subscription(topic='/amcl_pose',msg_type=PoseWithCovarianceStamped,callback=self.callback_amcl_pose,qos_profile=1000)
		...
		self.ws.connect('ws://127.0.0.1:8091')
...
def main(args=None):
	...
	log = './log.txt'
	actions = {}
	actions['/amcl_pose']=('log',0)
	actions['/monitoring_clock']=('log',0)
	monitor = ROSMonitor_monitor_prop11('monitor_prop11',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
