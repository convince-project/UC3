...
class ROSMonitor_monitor_prop13(Node):


	def callback_CheckNetworkComponent_NetworkStatus(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='/CheckNetworkComponent/NetworkStatus'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['/CheckNetworkComponent/NetworkStatus']={'package': 'std_msgs.msg', 'type': 'Bool'}
		self.topics_info['/monitoring_clock']={'package': 'builtin_interfaces.msg', 'type': 'Time'}
		self.config_subscribers['/CheckNetworkComponent/NetworkStatus']=self.create_subscription(topic='/CheckNetworkComponent/NetworkStatus',msg_type=Bool,callback=self.callback_CheckNetworkComponent_NetworkStatus,qos_profile=1000)
		...
		self.ws.connect('ws://127.0.0.1:8093')
...
def main(args=None):
	...
	log = './log.txt'
	actions = {}
	actions['/CheckNetworkComponent/NetworkStatus']=('log',0)
	actions['/monitoring_clock']=('log',0)
	monitor = ROSMonitor_monitor_prop13('monitor_prop13',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
