...
class ROSMonitor_monitor_prop5(Node):


	def callback_PeopleDetectorFilterComponent_filtered_detection(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='/PeopleDetectorFilterComponent/filtered_detection'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['/PeopleDetectorFilterComponent/filtered_detection']={'package': 'std_msgs.msg', 'type': 'Bool'}
		self.topics_info['/monitoring_clock']={'package': 'builtin_interfaces.msg', 'type': 'Time'}
		self.config_subscribers['/PeopleDetectorFilterComponent/filtered_detection']=self.create_subscription(topic='/PeopleDetectorFilterComponent/filtered_detection',msg_type=Bool,callback=self.callback_PeopleDetectorFilterComponent_filtered_detection,qos_profile=1000)
		...
		self.ws.connect('ws://127.0.0.1:8085')
...
def main(args=None):
	...
	log = './log.txt'
	actions = {}
	actions['/PeopleDetectorFilterComponent/filtered_detection']=('log',0)
	actions['/monitoring_clock']=('log',0)
	monitor = ROSMonitor_monitor_prop5('monitor_prop5',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
