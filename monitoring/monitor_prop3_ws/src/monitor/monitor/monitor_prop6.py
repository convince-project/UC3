...
class ROSMonitor_monitor_prop6(Node):


	def callback_cer_camera_repeater_publisher_status(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='/cer_camera_repeater/publisher_status'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['/cer_camera_repeater/publisher_status']={'package': 'std_msgs.msg', 'type': 'UInt8'}
		self.topics_info['/monitoring_clock']={'package': 'builtin_interfaces.msg', 'type': 'Time'}
		self.config_subscribers['/cer_camera_repeater/publisher_status']=self.create_subscription(topic='/cer_camera_repeater/publisher_status',msg_type=UInt8,callback=self.callback_cer_camera_repeater_publisher_status,qos_profile=1000)
		...
		self.ws.connect('ws://127.0.0.1:8086')
...
def main(args=None):
	...
	log = './log.txt'
	actions = {}
	actions['/cer_camera_repeater/publisher_status']=('log',0)
	actions['/monitoring_clock']=('log',0)
	monitor = ROSMonitor_monitor_prop6('monitor_prop6',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
