...
PROPERTY = r"historically( ({poi1_completed} -> {poi1_selected}) and ({poi1_selected} -> ( not( ((not {poi1_completed}) and {poi1_selected}) since[360:] {poi1_sel_start} ))))"
...
def abstract_message(message):
    ...
    if "topic" in message and "GetInt" in message['topic']:
        if "response" in message:
            for resp in message["response"]:
                if resp.get("field_name") == "PoiDone1" or resp.get("field_name") == "PoiDone2":
