...
class ROSMonitor_monitor_prop14(Node):


	def callback_CheckNetworkComponent_WebStatus(self,data):
		self.get_logger().info("monitor has observed "+ str(data))
		dict= rosidl_runtime_py.message_to_ordereddict(data)
		dict['topic']='/CheckNetworkComponent/WebStatus'
	...
	def __init__(self,monitor_name,log,actions):
		...
		self.topics_info['/CheckNetworkComponent/WebStatus']={'package': 'std_msgs.msg', 'type': 'Bool'}
		self.topics_info['/monitoring_clock']={'package': 'builtin_interfaces.msg', 'type': 'Time'}
		self.config_subscribers['/CheckNetworkComponent/WebStatus']=self.create_subscription(topic='/CheckNetworkComponent/WebStatus',msg_type=Bool,callback=self.callback_CheckNetworkComponent_WebStatus,qos_profile=1000)
		...
		self.ws.connect('ws://127.0.0.1:8094')
...
def main(args=None):
	...
	log = './log.txt'
	actions = {}
	actions['/CheckNetworkComponent/WebStatus']=('log',0)
	actions['/monitoring_clock']=('log',0)
	monitor = ROSMonitor_monitor_prop14('monitor_prop14',log,actions)
	rclpy.spin(monitor)
	monitor.ws.close()
