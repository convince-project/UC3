...
SayDurationWarningSkill::SayDurationWarningSkill(std::string name ) :
		m_name(std::move(name))
{
    
}

void SayDurationWarningSkill::spin(std::shared_ptr<rclcpp::Node> node)
{
	rclcpp::spin(node);
	rclcpp::shutdown();
}

bool SayDurationWarningSkill::start(int argc, char*argv[])
{
	if(!rclcpp::ok())
	{
	...
	RCLCPP_DEBUG_STREAM(m_node->get_logger(), "SayDurationWarningSkill::start");
	std::cout << "SayDurationWarningSkill::start";

    
	m_tickService = m_node->create_service<bt_interfaces::srv::TickAction>(m_name + "Skill/tick",
                                                                           	std::bind(&SayDurationWarningSkill::tick,
                                                                           	this,
                                                                           	std::placeholders::_1,
                                                                           	std::placeholders::_2));
    
	m_haltService = m_node->create_service<bt_interfaces::srv::HaltAction>(m_name + "Skill/halt",
                                                                            	std::bind(&SayDurationWarningSkill::halt,
                                                                            	this,
                                                                            	std::placeholders::_1,
                                                                            	std::placeholders::_2));
	...
	m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
		RCLCPP_INFO(m_node->get_logger(), "SayDurationWarningSkill::tickReturn %s", event.data().toMap()["result"].toString().toStdString().c_str());
	...
	m_stateMachine.connectToEvent("HALT_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
		RCLCPP_INFO(m_node->get_logger(), "SayDurationWarningSkill::haltresponse");
...
void SayDurationWarningSkill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces::srv::TickAction::Request> request,
                                std::shared_ptr<bt_interfaces::srv::TickAction::Response>      response)
{
    std::lock_guard<std::mutex> lock(m_requestMutex);
    RCLCPP_INFO(m_node->get_logger(), "SayDurationWarningSkill::tick");
    ...
    RCLCPP_INFO(m_node->get_logger(), "SayDurationWarningSkill::tickDone");
    response->is_ok = true;
}

void SayDurationWarningSkill::halt( [[maybe_unused]] const std::shared_ptr<bt_interfaces::srv::HaltAction::Request> request,
    [[maybe_unused]] std::shared_ptr<bt_interfaces::srv::HaltAction::Response> response)
{
    std::lock_guard<std::mutex> lock(m_requestMutex);
    RCLCPP_INFO(m_node->get_logger(), "SayDurationWarningSkill::halt");
    m_haltResult.store(false);
    m_stateMachine.submitEvent("CMD_HALT");
    while(!m_haltResult.load()) {
        std::this_thread::sleep_for (std::chrono::milliseconds(100));
    }
    RCLCPP_INFO(m_node->get_logger(), "SayDurationWarningSkill::haltDone");
    response->is_ok = true;
}
