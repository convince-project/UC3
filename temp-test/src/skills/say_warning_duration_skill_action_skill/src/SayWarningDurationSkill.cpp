...
SayWarningDurationSkill::SayWarningDurationSkill(std::string name ) :
...
bool SayWarningDurationSkill::start(int argc, char*argv[])
{
	...
	m_tickService = m_node->create_service<bt_interfaces_dummy::srv::TickActionCondition>(m_name + "Skill/tick",
                                                                           	std::bind(&SayWarningDurationSkill::tick,
                                                                           	this,
                                                                           	std::placeholders::_1,
                                                                           	std::placeholders::_2));
  
	m_haltService = m_node->create_service<bt_interfaces_dummy::srv::HaltActionCondition>(m_name + "Skill/halt",
  ...
  m_stateMachine.connectToEvent("TICK_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "SayWarningDurationSkill::tickReturn %s", event.data().toMap()["status"].toString().toStdString().c_str());
  ...
  m_stateMachine.connectToEvent("HALT_RESPONSE", [this]([[maybe_unused]]const QScxmlEvent & event){
    RCLCPP_INFO(m_node->get_logger(), "SayWarningDurationSkill::haltresponse");
...
void SayWarningDurationSkill::tick( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::TickActionCondition::Request> request,
                                std::shared_ptr<bt_interfaces_dummy::srv::TickActionCondition::Response>      response)
{
...
void SayWarningDurationSkill::halt( [[maybe_unused]] const std::shared_ptr<bt_interfaces_dummy::srv::HaltActionCondition::Request> request,
    [[maybe_unused]] std::shared_ptr<bt_interfaces_dummy::srv::HaltActionCondition::Response> response)
{
  std::lock_guard<std::mutex> lock(m_requestMutex);
  ...
  RCLCPP_INFO(m_node->get_logger(), "SayWarningDurationSkill::haltDone");
  response->is_ok = true;
}
